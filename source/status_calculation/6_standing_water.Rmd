...
mhq_su <- measurements_ws_all %>%
  ...
  filter(ws_type %in% c("mhq_sampleframe", "mhq_sample") | type == "3110") %>%
  ...
  left_join(ws_passend_beheer, by = "code_watersurfaces") %>%
...
mhq_su_recent <- mhq_su %>%
  filter(!is.na(area_ha)) %>%
  filter(ws_type == "mhq_sample" | type == "3110") %>%
  group_by(code_watersurfaces, type) %>%
  filter(date == max(date)) %>%
  ungroup() %>%
  anti_join(ws_main_type_disappeared, by = c("code_watersurfaces", "type")) %>%
...
```{r}
lsvi_habitat_ws <- lsvi_globaal %>%
  inner_join(select(mhq_su, id, date, type, main_type, in_sac, code_watersurfaces), by = c("id")) %>%
  mutate(lsvi = ifelse(status, "gunstig", "ongunstig")) %>%
  select(id, date, code_watersurfaces, in_sac, habitattype = main_type, habitatsubtype = type, lsvi, aggregatiemethode, n_zb_ongunstig, aandeel_gunstig, index_mean_ind) 

lsvi_habitat_ws %>%
  write_vc(root = output_path,
           file = str_c("lsvi_watersurface_", habcat_code),
           digits = 20,
           sorting = c("habitattype", "habitatsubtype", "date", "code_watersurfaces"),
           strict = FALSE)
```
...
status_flanders <- status_flanders_habt %>%
  ...
  mutate(area_gunstig_int_ha = ifelse(is.na(area_gunstig_int_ha), 0,  area_gunstig_int_ha),
         ...
         aandeel_gunstig = round((aandeel_gunstig/100 * area_sample_ha + area_gunstig_int_ha) / (area_measured_int_ha + area_sample_ha) * 100, 2),
         aandeel_gunstig_ulci = round((aandeel_gunstig_ulci/100 * area_sample_ha + area_gunstig_int_ha) / (area_measured_int_ha + area_sample_ha) * 100, 2),
         aandeel_gunstig_llci = round((aandeel_gunstig_llci/100 * area_sample_ha + area_gunstig_int_ha) / (area_measured_int_ha + area_sample_ha) * 100, 2),
  ...
  mutate(beoordeling = ifelse(habitattype == "3110", 
                              ifelse(aandeel_gunstig < 90, "Niet goed", "Goed"),
...
lsvi_habitat_beheer <- lsvi_habitat %>%
  inner_join(select(mhq_su_recent, id, weight_ws, weight_stratum, area_ha, area_ha_type, date, passend_beheer), by = "id") %>%
  mutate(weight = weight_ws) %>%
  mutate(weight = ifelse(is.na(weight), 1, weight),
         is_integraal = main_type == "3110" | 
                        area_class == ">= 50 ha" | 
...
lsvi_habitat_beheer_sample <- lsvi_habitat_beheer %>%
  filter(!is_integraal)

status_beheer_sample <- calc_status_passend_beheer(lsvi_habitat_beheer_sample)

```


```{r}
lsvi_habitat_beheer_integraal <- lsvi_habitat_beheer %>%
  filter(is_integraal)

lsvi_missing <- lsvi_missing %>%
  left_join(ws_passend_beheer, by = "code_watersurfaces")

lsvi_habitat_beheer_integraal <- lsvi_habitat_beheer_integraal %>%
  bind_rows(lsvi_missing)

beheer_integraal <- lsvi_habitat_beheer_integraal %>%
  group_by(main_type, passend_beheer) %>%
  summarise(area_unknown_ha = sum(area_ha_type * is.na(status)),
            area_measured_int_ha = sum(area_ha_type * !is.na(status)),
            area_gunstig_int_ha = sum(area_ha_type * status, na.rm = TRUE),
            n_obs_int = n_distinct(code_watersurfaces)) %>%
  ungroup() %>%
  rename( habitattype = main_type) %>%
  mutate(passend_beheer = ifelse(passend_beheer, "ja", "nee"))
...
areas_beheer_sample <- mhq_su_recent %>%
  filter(!(area_class == ">= 50 ha" | type == "3110" | (main_type == "2190" & area_class == "1 ha < area <= 5 ha"))) %>%
  group_by(main_type, passend_beheer) %>%
  summarise(area_sample_ha = sum(area_ha)) %>%
  ungroup() %>%
  rename(habitattype = main_type) %>%
  mutate(passend_beheer = ifelse(passend_beheer, "ja", "nee"))

ht3110_missing  <- status_beheer_sample %>%
  distinct(versie, type_resultaat, sbzh, passend_beheer) %>%
  mutate(habitattype = "3110",
         habitatsubtype = "3110",
         aandeel_gunstig = 0,
         n_obs = 0)
  
status_beheer_sample <- status_beheer_sample %>%
  bind_rows(ht3110_missing) %>%
  rename(n_obs_sample = n_obs)

status_beheer <- status_beheer_sample %>%
  left_join(areas_beheer_sample, by = c("habitattype", "passend_beheer")) %>%
  left_join(beheer_integraal, by = c("habitattype", "passend_beheer")) 
...
status_beheer <- status_beheer %>%
  mutate(area_gunstig_int_ha = ifelse(is.na(area_gunstig_int_ha), 0,  area_gunstig_int_ha),
         ...
         opp_onbekend_ha = round(area_unknown_ha, 2),
         opp_totaal_ha = opp_uitspraak_ha + opp_onbekend_ha) %>%
  select(-area_gunstig_int_ha, -area_measured_int_ha, -area_unknown_ha, -area_sample_ha, -p_waarde_beheer, -effect_beheer) %>%
  mutate(beoordeling = ifelse(habitattype == "3110", 
                              ...
                              ifelse(is.na(aandeel_gunstig_ulci), "Onbekend",
                                ifelse(aandeel_gunstig_llci >= 75, "Goed",
                                       ifelse(aandeel_gunstig_ulci < 75, "Niet goed",
                                              "Onbekend"))))) %>%
  filter(!is.na(beoordeling)) %>%
  select(type_resultaat, versie, habitattype, sbzh, passend_beheer, habitatsubtype, n_obs_sample, n_obs_int, n_obs, everything())
...
lsvi_indicator_recent_sample <- lsvi_indicator_recent %>%
  filter(!is_integraal)

status_indicatoren_sample <- calc_status_indicator(lsvi_indicator_recent_sample) %>%
  mutate(habitattype = factor(habitattype, levels = levels(types_habcat$type)))
...
```{r}
lsvi_indicator_recent_integraal <- lsvi_indicator_recent %>%
  filter(is_integraal)

indicatoren_ws <- lsvi_indicator %>%
  distinct(type, criterium, indicator, belang)

lsvi_indicator_missing <- lsvi_missing %>%
  left_join(indicatoren_ws, by = "type")

lsvi_indicator_recent_integraal <- lsvi_indicator_recent_integraal %>%
  bind_rows(lsvi_indicator_missing)

maintype_indicator_integraal <- lsvi_indicator_recent_integraal %>%
  group_by(main_type, indicator) %>%
  summarise(area_unknown_ha = sum(area_ha_type * is.na(status_indicator)),
            area_measured_int_ha = sum(area_ha_type * !is.na(status_indicator)),
            area_gunstig_int_ha = sum(area_ha_type * status_indicator, na.rm = TRUE),
            n_obs_int = n_distinct(code_watersurfaces)) %>%
  ungroup() %>%
  rename(habitattype = main_type)
...
ht3110_missing  <- status_indicatoren_sample %>%
  distinct(versie, schaal, type_resultaat, sbzh) %>%
  mutate(habitattype = "3110",
         habitatsubtype = "3110",
         aandeel_gunstig = 0,
...
status_indicatoren <- status_indicatoren_sample %>%
  filter(type_resultaat == "Habitattype") %>%
  bind_rows(ht3110_missing) %>%
  left_join(areas_habt_sample, by = "habitattype") %>%
  left_join(maintype_indicator_integraal, by = c("habitattype", "indicator")) %>%
  mutate(area_gunstig_int_ha = ifelse(is.na(area_gunstig_int_ha), 0,  area_gunstig_int_ha),
         area_measured_int_ha = ifelse(is.na(area_measured_int_ha), 0,  area_measured_int_ha),
         area_unknown_ha = ifelse(is.na(area_unknown_ha), 0,  area_unknown_ha),
         area_sample_ha = ifelse(is.na(area_sample_ha), 0,  area_sample_ha),
...
status_indicatoren %>%
  write_vc(root = output_path,
           file = str_c("status_indicatoren_", habcat_code),
           sorting = c("habitattype", "type_resultaat"),
           strict = FALSE)
```
