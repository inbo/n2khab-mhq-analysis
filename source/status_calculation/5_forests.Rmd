...
data_habitat_mhq <- data_habitat %>%
  filter(survey == "anb") %>%
  select(id = ID, type_analysis = Habitattype, date, record_id = record_id_square, link_mhq)

data_habitata_vbi <- data_habitat %>%
  filter(survey == "vbi") %>%
  select(id = ID, type_analysis = Habitattype, phab, date, link_mhq = record_id_square, vbi_cycle)

data_habitat_moneos <- data_habitat_moneos %>%
  select(id = ID, type_analysis = Habitattype, date, record_id = record_id_square, link_mhq) %>%
  filter(type_analysis == "91E0_sf")

data_habitat <- bind_rows(data_habitat_mhq,
                          data_habitata_vbi,
                          data_habitat_moneos)
...
mhq_measurements_habcat <- mhq_measurements %>%
  ...
  inner_join(data_habitat_mhq, by = c("link_mhq", "date"))

check_missing_lsvi <- mhq_measurements %>%
  semi_join(types_habcat, by = "type") %>%
  rename(date = measurement_date) %>%
  anti_join(data_habitat_mhq, by = c("link_mhq", "date"))
...
moneos_su <- data_habitat_moneos %>%
  mutate(type = type_analysis,
         main_type = "91E0",
         point_code = link_mhq) %>%
  left_join(strata_91E0_sf, by = "link_mhq") %>%
  left_join(stratum_passend_beheer_91E0_sf, by = "link_mhq")

vbi_su <- data_habitata_vbi %>%
  mutate(phab = ifelse(is.na(phab), 100, phab)) %>%
  group_by(date, link_mhq, vbi_cycle) %>%
  filter(phab == max(phab)) %>%
  ungroup() %>%
  mutate(type = type_analysis,
         main_type = str_sub(type, 1, 4)) %>%
  left_join(strata_vbi, by = c("link_mhq", "vbi_cycle")) %>%
  left_join(select(stratum_passend_beheer_vbi, passend_beheer, link_mhq, vbi_cycle), by = c("link_mhq", "vbi_cycle")) %>%
  mutate(distance_2_3 = round(distance_2_3, 1)) %>%
  select(id, point_code, link_mhq, x, y, vbi_cycle, date, main_type, type, type_analysis, phab, in_sac, passend_beheer, distance_2_3)
  
check_in_sac <- vbi_su %>%
  group_by(point_code) %>%
  filter(n_distinct(in_sac) > 1)

# 1 punt vbi_318089 bij eerste meting binnen SBZH, bij tweede meting buiten SBZH
vbi_su <- vbi_su %>%
  group_by(point_code) %>%
  mutate(in_sac = any(in_sac)) %>%
  ungroup()

check_type <- vbi_su %>%
  group_by(point_code, vbi_cycle) %>%
...
```{r}
lsvi_habitat_plot <- lsvi_globaal %>%
  inner_join(select(mhq_su, id, date, type_analysis, type, main_type, in_sac, point_code, x, y), by = c("id", "type_analysis")) %>%
  mutate(lsvi = ifelse(status, "gunstig", "ongunstig")) %>%
  select(id, date, point_code, x, y, in_sac, habitattype = main_type, habitatsubtype = type, lsvi, aggregatiemethode, n_zb_ongunstig, aandeel_gunstig, index_mean_ind, doelbereik_mean_ind) 

lsvi_habitat_plot %>%
  write_vc(root = output_path,
           file = str_c("lsvi_plot_", habcat_code),
           digits = 20,
           sorting = c("habitattype", "habitatsubtype", "date", "point_code"),
           strict = FALSE)
```
...
index_ind <- calc_index_indicator(lsvi_indicator_recent)

index_ind %>%
  write_vc(root = output_path,
           file = str_c("index_ind_", habcat_code),
...
mhq_su_remeasured %>%
  ...
  select(habitattype = main_type, meetcyclus = periode, periode = periode_year, n_pq) %>%
  kable() %>%
  kable_styling() %>%
...
mhq_su_remeasured <- mhq_su_remeasured %>%
  filter(periode != "extra") %>%
  group_by(main_type, periode) %>%
  mutate(n_pq_main_type = n_distinct(point_code)) %>%
  ungroup() %>%
  filter(n_pq_main_type >= nmin_trend)
```
...
```{r, eval = FALSE}
lsvi_habitat_trend_orig <- read_vc(root = output_path,
           file = str_c("trend_habitat_", habcat_code))

lsvi_habitat_trend <- lsvi_habitat_trend_orig %>%
...
trend_flanders_glmm <- lsvi_habitat_trend %>%
  filter(type_resultaat == "Habitattype") %>%
  filter(parameter != "aandeel_gunstig") %>%
  select(habitattype, parameter, mean, llci_0.95, ulci_0.95, klasse) %>%
  mutate(model = "glmm inla")
...
overzicht_verschil <- trend_flanders_glmm %>%
  bind_rows(trend_flanders_glm) %>%
  arrange(habitattype, parameter)
...
check_periodes <- lsvi_indicatoren_remeasured %>%
  group_by(main_type, periode, indicator) %>%
  summarise(n = n(),
            type = str_c(unique(type_analysis), collapse = "; "),
            year_min = year(min(date)),
            year_max = year(max(date))) %>%
  ungroup()
...
```{r, eval = FALSE}
trend_indicatoren_orig <- read_vc(root = output_path,
           file = str_c("trend_indicatoren_", habcat_code))

lsvi_indicator_trend <- trend_indicatoren_orig %>%
  filter(habitattype != "9110")
```
...
lsvi_indicator_trend <- calc_trend_indicator(lsvi_indicatoren, re_location = FALSE) %>%
  arrange(habitattype)
...
```{r}
data_voorwaarden <- read_vc(root = file.path(path_lsvi, str_c("lsvi_mhq/", habcat_name, "/input")),
                             file = str_c("data_voorwaarden_", habcat_code))

data_voorwaarden_moneos <- read_vc(root = file.path(path_lsvi, "lsvi_mhq/moneos/input"),
                             file = "data_voorwaarden_moneos")

dik_dood_hout <- data_voorwaarden %>%
  bind_rows(data_voorwaarden_moneos) %>%
  filter(Indicator == "hoeveelheid dik dood hout") %>%
  rename(id = ID)

dik_dood_hout_recent <- dik_dood_hout %>%
  inner_join(select(mhq_su_recent, id, type, type_analysis, weight, in_sac, date), by = "id") 

dik_dood_hout_remeasured <- dik_dood_hout %>%
  inner_join(select(mhq_su_remeasured, id, point_code, type, type_analysis, weight, date, periode, diff_years, in_sac), by = "id") 
...
```{r}
dik_dood_hout_status <- dik_dood_hout_recent %>%
  mutate(Versie = "Versie 3",
         WeightComb = weight,
         PlotWeight = 1,
         StratumWeight = weight,
         Habitatsubtype = type_analysis,
         Habitattype = str_sub(type_analysis, 1, 4),
         SBZH = ifelse(in_sac, "Binnen", "Buiten")) %>%
  filter(Habitattype != "9110") %>%
  habitatGemiddeldeVW()

dik_dood_hout_status <- dik_dood_hout_status %>%
  filter(SBZH == "Binnen & Buiten") %>%
  mutate(schaal = "Vlaanderen") %>%
  select(Versie, type_resultaat, schaal, Indicator, Voorwaarde, Habitattype, Habitatsubtype, SBZH, n_obs = nObs, Gemiddelde, Gemiddelde_LLCI, Gemiddelde_ULCI) %>%
  arrange(Habitattype)

colnames(dik_dood_hout_status) <- str_to_lower(colnames(dik_dood_hout_status))

dik_dood_hout_status %>%
  write_vc(root = output_path,
           file = str_c("status_dikdoodhout"),
           sorting = c("habitattype", "type_resultaat", "sbzh", "habitatsubtype", "indicator"),
           strict = FALSE)

dik_dood_hout_status %>%
  write_csv2(str_c(output_path, "/status_dikdoodhout.csv"))
```

### Trend

```{r}
dik_dood_hout_trend <- dik_dood_hout_remeasured %>%
    mutate(Versie = "Versie 3.0",
         WeightComb = weight,
         PlotWeight = 1,
         StratumWeight = weight,
         Habitatsubtype = type_analysis,
         Habitattype = str_sub(type_analysis, 1, 4),
         SBZH = ifelse(in_sac, "Binnen", "Buiten")) %>%
  select(-date, -record_id, -id) %>%
  pivot_wider(names_from = periode,
              values_from = Waarde) %>%
  mutate(Waarde = c_2 - c_1) %>%
  filter(Habitattype != "9110") %>%
  habitatGemiddeldeVW()

dik_dood_hout_trend <- dik_dood_hout_trend %>%
  filter(SBZH == "Binnen & Buiten") %>%
  mutate(schaal = "Vlaanderen") %>%
  select(Versie, type_resultaat, schaal, Indicator, Voorwaarde, Habitattype, Habitatsubtype, SBZH, n_obs = nObs, Verschil = Gemiddelde, Verschil_LLCI = Gemiddelde_LLCI, Verschil_ULCI = Gemiddelde_ULCI) %>%
  arrange(Habitattype)

colnames(dik_dood_hout_trend) <- str_to_lower(colnames(dik_dood_hout_trend))

dik_dood_hout_trend %>%
  write_vc(root = output_path,
           file = str_c("trend_dikdoodhout"),
           sorting = c("habitattype", "type_resultaat", "sbzh", "habitatsubtype", "indicator"),
           strict = FALSE)

dik_dood_hout_trend %>%
  write_csv2(str_c(output_path, "/trend_dikdoodhout.csv"))
```
