# Heide en inlandse duinen

```{r}
types_habcat <- read_types() %>%
  filter(typeclass %in% c("HS", "ID"))

habcat_name <- "heath_inland_dunes"
habcat_code <- "hs_id"
```
...
# remove indicators with less than 5 repeated measurements
indicator_remove <- lsvi_indicatoren_remeasured %>%
...
lsvi_indicatoren_remeasured <- lsvi_indicatoren_remeasured %>%
  anti_join(indicator_remove, by = "indicator")
