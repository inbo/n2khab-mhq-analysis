...
mhq_su <- measurements_streams_mhq %>%
  ...
  select(id, segment_id, in_sac, main_type, type, date, latitude, longitude) 
...
mhq_su_recent <- mhq_su %>%
  ...
  ungroup() %>%
  anti_join(ht3260_disappeared, by = "segment_id") %>%
...
lsvi_habitat_plot <- lsvi_globaal %>%
  inner_join(select(mhq_su, id, date, type, main_type, in_sac, segment_id, latitude, longitude), by = c("id")) %>%
  left_join(select(mhq_streams_samplingframe, segment_id, x, y)) %>%
  mutate(lsvi = ifelse(status, "gunstig", "ongunstig")) %>%

  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>%
  st_transform(crs = 31370) %>%
  mutate(x_measured = st_coordinates(.data$geometry)[,1],
         y_measured = st_coordinates(.data$geometry)[,2]) %>%
  st_drop_geometry() %>%
  mutate(x = ifelse(!is.na(x_measured), x_measured, x),
         y = ifelse(!is.na(y_measured), y_measured, y)) %>%
    select(id, date, segment_id, x, y, in_sac, habitattype = main_type, habitatsubtype = type, lsvi, aggregatiemethode, n_zb_ongunstig, aandeel_gunstig, index_mean_ind)
 
lsvi_habitat_plot %>%
  write_vc(root = output_path,
           file = str_c("lsvi_plot_", habcat_code),
           digits = 20,
           sorting = c("habitattype", "habitatsubtype", "date", "segment_id"),
           strict = FALSE)
```
...
lsvi_habitat_remeasured <- lsvi_habitat %>%
  inner_join(select(mhq_su_remeasured, id, weight = weight_stratum_scaled, date, periode, diff_years), by = "id")
