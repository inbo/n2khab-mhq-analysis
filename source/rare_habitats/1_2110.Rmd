...
status_indicator_2110 <- status_indicator_vl_2110 %>%
  ...
  mutate(versie = "Versie 3",
         schaal = "Vlaanderen") %>%
  select(versie, schaal, type_resultaat, habitattype, sbzh, criterium, indicator, aandeel_gunstig = aandeel_gunstig_min, beoordeling, opp_gunstig_ha, opp_ongunstig_ha, opp_onbekend_ha)
...
trend_indicatoren_2110 <- status_indicator_vl_2110 %>%
  bind_rows(status_indicator_sbzh_2110) %>%
  select(versie_kartering, habitattype, sbzh, type_resultaat, criterium, indicator, aandeel_gunstig) %>%
  pivot_wider(names_from = versie_kartering,
              values_from = aandeel_gunstig,
              names_prefix = "aandeel_gunstig_") %>%
  mutate(mean = round(aandeel_gunstig_2023 - aandeel_gunstig_2017, 2),
         ...
         versie = "Versie 3",
         schaal = "Vlaanderen") %>%
  select(versie, schaal, type_resultaat, habitattype, sbzh, criterium, indicator, parameter, mean, klasse, jaar_min, jaar_max, verschil_jaar)
