...
Voorwaarde bosconstantie: gunstig indien bosleeftijd > 30 jaar.
Hoe bepalen?

Miniumum structuurareaal. 

```{r}

coordinates_91E0_sf <- coordinates_91E0_sf %>%
  filter(str_detect(plot_pl, "a")) %>%
  select(plot_id, x, y)

data_habitat_91E0_sf <- data_habitat_moneos %>%
  filter(type_observed == "91E0_sf") %>%
  select(ID, record_id_square, plot_id, date, type_observed) %>%
  left_join(coordinates_91E0_sf, by = "plot_id")

data_path_extravar <- file.path(fileman_up("n2khab-mhq-data"), "/raw")

plot_coord <- data_habitat_91E0_sf %>%
    filter(!is.na(x)) %>%
