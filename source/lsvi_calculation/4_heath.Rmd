...
'Open vegetaties en kaalzand' is de dom van 'open vegetaties' en open bodem.
...
+ bij 2330_bu is de naam van de voorwaarde 'open vegetaties' ipv 'open vegetaties en kaal zand' en ontbreekt open zand bij studiewaarden
...
correct_status_criterium <- lsvi_hs_id$Resultaat_indicator %>%
  filter(!is.na(Status_indicator)) %>%
...
lsvi_hs_id$Resultaat_globaal <- lsvi_hs_id$Resultaat_globaal %>%
  left_join(correct_status_globaal, by = c("ID", "Habitattype")) %>%
  mutate(Status = status_habitatvlek) %>%
  select(-status_habitatvlek)

lsvi_hs_id$Resultaat_criterium <- lsvi_hs_id$Resultaat_criterium %>%
...
lsvi_hs_id %>%
  write_lsvi_results(path = output_path)
```
